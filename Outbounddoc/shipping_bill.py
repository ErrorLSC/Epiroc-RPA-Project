...
def consolidate_shipment(pickdf,name_block_list=None,address_block_list=None,fixed_consolidate_dict=None):
    ...
    #other_df['STDADD'] = other_df['SHIPADD'].apply(lambda x:standardize_address(x))
    #print(other_df['STDADD'])
    ...
    #other_df.to_csv('outbounddoc/other_df.csv', encoding='utf_8_sig')
...
def template_fulfillment(excel_template,shipment_diretion,outputpath):
    ...
    for i in range(1,len(shipment_all)+1,1):
        j = str(i)
        shipment_direction_dict_perpicktime = shipment_diretion.get_shipment_by_picktime(j)
        # 件数
    ...
    consolidated_shipment_dict = shipment_diretion.get_consolidated_shipment_orders()
    # 定义起始行和列
    start_row = 45
    key_col = 3  # C 列
    value_col = 4  # D 列
...
if __name__ == '__main__':
    csv0 = r"\\ssisjpfs0004\JPN\MRBA\Logistics\CMT Logistics\FromBPCS\DOWNLOADS\Yamato\送信済みデータ\lypl20240725 1100.csv"
    csv1 = r"\\ssisjpfs0004\JPN\MRBA\Logistics\CMT Logistics\FromBPCS\DOWNLOADS\Yamato\送信済みデータ\lypl20240725 1430.csv"
    csv2 = r"\\ssisjpfs0004\JPN\MRBA\Logistics\CMT Logistics\FromBPCS\DOWNLOADS\Yamato\送信済みデータ\lypl20240725 1530.csv"
    #csv3 = r"\\ssisjpfs0004\JPN\MRBA\Logistics\CMT Logistics\FromBPCS\DOWNLOADS\Yamato\送信済みデータ\lypl20240723 1530.csv"

    tempcsv = "Outbounddoc\pick0725.csv"
