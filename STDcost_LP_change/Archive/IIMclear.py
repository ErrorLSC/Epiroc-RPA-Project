...
def read_or_create_filteredIIM(filteredIIMpath, IIMcleaning,IIMpath):
    if os.path.exists(filteredIIMpath):
        print(f"File {filteredIIMpath} exists. Reading file...")
        df = pd.read_excel(filteredIIMpath,dtype={"IPROD":str})
        return df
    else:
        print(f"File {filteredIIMpath} does not exist. Creating file...")
        IIMcleaning(IIMpath,filteredIIMpath,fileoutput=True)
        df = pd.read_excel(filteredIIMpath,dtype={"IPROD":str})
        return df

def IIMcleaning(IIMpath,outputpath,fileoutput=True):
    IIMdf = pd.read_excel(IIMpath,dtype={"IPROD":str})
    IIMdf['IPROD'] = IIMdf['IPROD'].str.strip()
    IIMdf['IXRATG'] = IIMdf['IXRATG'].str.strip()
...
def rategroup_update(filteredIIMdf,EPLALL,output,fileoutput=True):
    ...
    if fileoutput is True:
        with pd.ExcelWriter(output) as writer:
            filteredIIMdf_newRG_notNN.to_excel(writer, sheet_name='Not_NN', index=False)
            filteredIIMdf_newRG_NN.to_excel(writer, sheet_name='NN', index=False)
...
def PLC_filtering(PLCpath,filteredIIM_newRG_notNN,PLCname,outputpath):
    ...
    final_outputpath = outputpath + "\\Items_newRG_inEPL" + PLCname + ".xlsx"
    IIM_PLC_df.to_excel(final_outputpath,index=False)

if __name__ == "__main__":
    IIMpath = r"C:\Users\jpeqz\OneDrive - Epiroc\SCX\QRYs\Master File\Item Master.xlsx"
    ...
    filteredIIMdf_newRG_output = r"C:\Users\jpeqz\OneDrive - Epiroc\SCX\DATA brush up\LISTPRICE&STD COST\EPL\filteredIIMdf_newRG.xlsx"
    ...
    filteredIIMdf = read_or_create_filteredIIM(filteredIIMpath, IIMcleaning,IIMpath)
